...
class RatingBar: UIView {
    ...
    func buildView(){
        ...
        //加入滑动手势
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(panRateView(_:)))
    ...
    //点击编辑分数后，通过手势的x坐标来设置数值
    func panRateView(sender: UIPanGestureRecognizer){
        if isIndicator {return}//如果是指示器，就不能交互
        
        if sender.state == .Began {
            panStartX = sender.locationInView(self).x
        } else if sender.state == .Changed {
            let offset = sender.translationInView(self).x + panStartX
            //通过x坐标判断分数
            let realRatingScore = offset / (self.bounds.size.width / ratingMax);
            self.rating = self.incomplete ? realRatingScore : round(realRatingScore)
        } else {
