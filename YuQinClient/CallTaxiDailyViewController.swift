...
class CallTaxiDailyViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
//        tableView.backgroundColor = UIColor.clearColor()
        ...
        //注册通知
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(didSelectCustomer(_:)), name: Constant.CallTaxiDailyViewControllerDidSelectCustomerNofification, object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(didSelectStartAddress(_:)), name: Constant.CallTaxiDailyViewControllerDidSelectStartAddressNofification, object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(didSelectFavStartAddress(_:)), name: Constant.CallTaxiDailyViewControllerDidSelectFavStartAddressNofification, object: nil)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        case 1:
            ...
            if indexPath.row == 0 {
                beginDateLabel = cell1.dateLabel
                beginDateLabel.text = beginDate != nil ? dateFormatter.stringFromDate(beginDate!) : "请选择开始时间"
            } else {
                endDateLabel = cell1.dateLabel
                endDateLabel.text = beginDate != nil ? dateFormatter.stringFromDate(endDate!) : "请选择结束时间"
            }
        case 2:
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        case 0:
            ...
            page2.identifier = Constant.CallTaxiDailyViewControllerDidSelectCustomerNofification
            page2.navigationItem.hidesBackButton = true
            self.navigationController?.pushViewController(page2, animated: true)
            break
        case 1:
            if indexPath.row == 0 {
                selectDateForLabel(beginDateLabel)
            } else {
                selectDateForLabel(endDateLabel)
            }
        case 2:
            let page2 = storyboard?.instantiateViewControllerWithIdentifier("SelectAddressViewController") as! SelectAddressViewController
            page2.identifier = Constant.CallTaxiDailyViewControllerDidSelectStartAddressNofification
            self.navigationController?.pushViewController(page2, animated: true)
        case 3:
            break
        default:
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        ...
        if fromAddress != nil && beginDate != nil && endDate != nil {
            estimatePrice()
        }
    }
        ...
        let datePicker = ActionSheetDatePicker(title: "", datePickerMode: UIDatePickerMode.DateAndTime, selectedDate: NSDate(), doneBlock: {
            ...
            if label == self.beginDateLabel {
                ...
                if self.beginDate != nil && endDate?.compare(self.beginDate!) == .OrderedAscending {
                    ...
                    UITools.sharedInstance.toast(toView: self.view, labelText: "结束日期不能小于开始日期")
                    return
                }
                self.endDate = endDate
                let dateStr = self.dateFormatter.stringFromDate(endDate!)
                label.text = dateStr
                //估价
...
//        }
    ...
    func popupSelectFavoriteAddressController(sender: UIButton) {
        ...
        if sender.tag == 0 {
            vc.identifier = Constant.CallTaxiDailyViewControllerDidSelectFavStartAddressNofification
        } else {
            vc.identifier = Constant.CallTaxiDailyViewControllerDidSelectFavEndAddressNofification
        }
        vc.title = "请选择常用地址"
...
//            Tools.sharedInstance.logout(withToast: true)
    ...
    func estimatePrice() {
        ...
        let parameters = ["phoneNumber"                     : phoneNumber,
                          ...
                          "beginAddress"                    : "",
                          "beginAddressDetail"              : "",
                          "beginAddressLocationLongitude"   : "",
                          "beginAddressLocationLatitude"    : "",
                          "endAddress"                      : "",
                          "endAddressDetail"                : "",
                          "endAddressLocationLongitude"     : "",
                          "endAddressLocationLatitude"      : ""]
                ...
                case .Success(let value):
                    ...
                    if let status = value["status"] as? String {
                        ...
                        if let price = dict["price"] as? Int, quantity = dict["quantity"] as? Int {
                            self.bottomView.estimatedCostLabel.text = "¥\(price)（\(quantity)天）"
                        }
                    }
                    break
                case .Failure(let error):
    ...
    func submitOrder() {
        ...
        if beginDate == nil{
            UITools.sharedInstance.shakeView(beginDateLabel)
            return
        }
        if endDate == nil{
            UITools.sharedInstance.shakeView(endDateLabel)
            return
        }
        if fromAddress == nil{
